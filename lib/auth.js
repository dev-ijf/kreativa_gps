...
export async function createAdmin(payload) {
  const username = resolveAdminUsername(payload);
  const password = payload.password?.trim() || randomBytes(24).toString('base64url');
...
export async function updateAdmin(id, payload) {
  ...
  if (payload.password) {
    values.push(hashAdminPassword(payload.password.trim()));
