...
function getAttendedTicketCount(rows) {
  return rows
    .filter(row => (row.status || row.registration_status) === 'attended')
    .reduce((total, row) => total + normalizeCount(row.attendeeCount || row.attendee_count, 1), 0);
}
...
export async function getConfig() {
  ...
  `);
  const usedTickets = Number(result.rows[0]?.used_tickets || 0);
...
export async function checkExistingGeneralRegistration({ participantName, phone, email, excludeRegistrationId = null }) {
  const pool = getPool();
  await ensureVerificationSchema(pool);
  const excludedId = normalizeText(excludeRegistrationId);
