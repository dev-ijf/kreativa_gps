...
function checkJsonExistingGeneralRegistration(store, { participantName, phone, email, excludeRegistrationId = null }) {
  const excludedId = normalizeText(excludeRegistrationId);
  ...
  if (!normalizedName || (!normalizedPhone && !normalizedEmail)) {
    return {
      exists: false,
      registrationId: null,
      notes: 'No existing general registration found.'
    };
  }

  const matched = store.registrations
    .filter(row => row.parentCategory === 'general'
      && row.id !== excludedId
      && row.registrationId !== excludedId
      && row.verificationStatus === 'verified'
      && normalizeName(row.studentName) === normalizedName
      && (
        (normalizedPhone && normalizePhoneDigits(row.phone) === normalizedPhone)
  ...
  if (matched) {
    return {
      exists: true,
      registrationId: matched.id,
      notes: `Duplicate general registration found by participant name + contact. Existing registration ID: ${matched.id}`
    };
  }
  ...
  return {
    ...
    async create(payload) {
      ...
      if (verification.status === 'verified') {
        const duplicateCheck = isGeneral
          ? checkJsonExistingGeneralRegistration(store, {
            participantName: payload.studentName,
            phone: payload.phone,
            email: payload.email,
    ...
    async submitPaymentProof(payload) {
      ...
      const duplicateCheck = current.parentCategory === 'general'
        ? checkJsonExistingGeneralRegistration(store, {
          participantName: current.studentName,
          phone: current.phone,
          email: current.email,
...
async function createPostgresRepository() {
  ...
  async function checkExistingGeneralRegistration({ participantName, phone, email, excludeRegistrationId = null }) {
    await ensureVerificationSchema(pool);
    const excludedId = normalizeText(excludeRegistrationId);
    const normalizedName = normalizeName(participantName);
    const normalizedPhone = normalizePhoneDigits(phone);
    ...
    const params = [normalizedName, normalizedPhone, normalizedEmail];
    const excludeClause = excludedId
      ? `AND id::text <> $${params.push(excludedId)}::text AND registration_id <> $${params.length}::text`
      : '';
    const result = await pool.query(
      `SELECT id, registration_id, student_name, parent_category, verification_status, payment_status, created_at
       FROM registrations
       WHERE parent_category = 'general'
         ${excludeClause}
         AND verification_status = 'verified'
         AND LOWER(TRIM(REGEXP_REPLACE(student_name, '\\s+', ' ', 'g'))) = $1::text
         AND (
           ($2::text <> '' AND REGEXP_REPLACE(COALESCE(phone, ''), '\\D', '', 'g') = $2::text)
           OR ($3::text <> '' AND LOWER(TRIM(COALESCE(email, ''))) = $3::text)
         )
         AND (
           payment_status IN ('verified', 'paid', 'confirmed', 'waiting_confirmation')
           OR (
    ...
    if (result.rows.length > 0) {
      return {
        exists: true,
        registrationId: result.rows[0].id,
        notes: `Duplicate general registration found by participant name + contact. Existing registration ID: ${result.rows[0].id}`
      };
    }
  ...
  return {
    ...
    async create(payload) {
      ...
      if (verification.status === 'verified') {
        const duplicateCheck = isGeneral
          ? await checkExistingGeneralRegistration({
            participantName: payload.studentName,
            phone: payload.phone,
            email: payload.email,
            excludeRegistrationId: reusableDraft ? draftRegistration.id : null
    ...
    async submitPaymentProof(payload) {
      ...
      const duplicateCheck = current.parent_category === 'general'
        ? await checkExistingGeneralRegistration({
          participantName: current.student_name,
          phone: current.phone,
          email: current.email,
          excludeRegistrationId: current.id
        })
